      ...
      <div className="min-h-screen p-4 pb-10" style={{ background: "#0a1220", backgroundImage: "radial-gradient(ellipse 80% 50% at 50% -10%, #1a2f4e 0%, #0a1220 65%)", ...theme } as React.CSSProperties}>
        <div className="max-w-sm mx-auto pt-8 space-y-6 text-center">
          ...
          <p className="text-xs text-slate-500 rounded-xl px-4 py-3" style={{ background: "rgba(13,27,42,0.70)", border: "1px solid rgba(255,255,255,0.07)" }}>
            Status: <span style={{ color: "var(--accent)" }}>aguardando aprovação da equipe</span> para retirada/entrega.
          </p>
          {pdfUrl && (
            <a
              href={pdfUrl}
        ...
        {step === "dados" && (
          <form onSubmit={goToTermo} className="space-y-4">
            <div className="rounded-2xl p-5 space-y-4" style={{ background: "rgba(13,27,42,0.70)", border: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-300">CPF <span style={{ color: "var(--accent)" }}>*</span></label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-300">E-mail <span style={{ color: "var(--accent)" }}>*</span> <span className="text-slate-500">(avisamos que recebemos seu pedido)</span></label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" autoComplete="email"
                  className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 outline-none" style={inputStyle} />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-300">CEP <span style={{ color: "var(--accent)" }}>*</span> <span className="text-slate-500">(preenche o resto)</span></label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2 space-y-1.5">
                  <label className="text-xs font-medium text-slate-300">Rua <span style={{ color: "var(--accent)" }}>*</span></label>
                  <input type="text" value={logradouro} onChange={(e) => setLogradouro(e.target.value)} placeholder="Nome da rua"
                    className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 outline-none" style={inputStyle} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-300">Nº <span style={{ color: "var(--accent)" }}>*</span></label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-300">Bairro <span style={{ color: "var(--accent)" }}>*</span></label>
                  <input type="text" value={neighborhood} onChange={(e) => setNeighborhood(e.target.value)} placeholder="Seu bairro"
                    className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 outline-none" style={inputStyle} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-300">Cidade <span style={{ color: "var(--accent)" }}>*</span></label>
                  <input type="text" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Ex: Curitiba"
                    className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 outline-none" style={inputStyle} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-300">UF <span style={{ color: "var(--accent)" }}>*</span></label>
        ...
        {step === "termo" && (
          <div className="space-y-4">
            ...
            <label
              ...
              style={{
                background: termAccepted ? "rgba(var(--accent-rgb),0.06)" : "rgba(13,27,42,0.50)",
                border: termAccepted ? "1px solid rgba(var(--accent-rgb),0.25)" : "1px solid rgba(255,255,255,0.07)",
              }}
            >
              <div className="mt-0.5 shrink-0 relative">
                <input type="checkbox" checked={termAccepted} onChange={(e) => setTermAccepted(e.target.checked)} className="sr-only" />
                <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: termAccepted ? "var(--accent)" : "transparent", border: termAccepted ? "1px solid var(--accent)" : "1px solid rgba(255,255,255,0.2)" }}>
              ...
              <p className="text-xs leading-relaxed" style={{ color: termAccepted ? "rgba(var(--accent-rgb),0.9)" : "#94a3b8" }}>
                Li e concordo com o Termo de Apoiador acima, e confirmo que os dados informados (nome e CPF) são
                verdadeiros e servem como minha assinatura eletrônica neste documento. <span style={{ color: "var(--accent)" }}>*</span>
              </p>
            </label>
