...
const ROLE_ORDER = ["COORD_GERAL", "COORD_REGIONAL", "LIDER_MUNICIPAL", "LIDER_BAIRRO", "VOLUNTARIO"];
...
export default async function RelatorioPage() {
  ...
  const collaborators = await db.collaborator.findMany({
    ...
    select: { city: true, campaignRole: true, status: true, supportStatus: true },
  });
  ...
  const totals = cities.reduce((acc, [, m]) => ({
    ...
    confirmados: acc.confirmados + m.confirmados,
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.08]" style={{ background: "rgba(13,27,42,0.5)" }}>
                ...
                {ROLE_ORDER.map((r) => (
                  <th key={r} className="text-center px-3 py-3 text-xs font-semibold text-muted-foreground">{ROLE_LABEL[r]}</th>
                ))}
